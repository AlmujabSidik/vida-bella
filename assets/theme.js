  ...
  theme.buildProductGridItem = function (items, gridWidth, rowOf, imageSizes) {
    ...
    items.forEach((product) => {
      ...
      if (theme.settings.predictiveSearchPrice)
        priceMarkup = `<div class="grid-product__price">${theme.strings.productFrom}${theme.Currency.formatMoney(product.price_min, theme.moneyFormat)}</div>`;
...
setTimeout(() => {
  const elements = document.getElementsByClassName("swatch-option-name");
  console.log("object", elements[1]);
  const div = document.createElement("div");
  div.className = "kiwi";
  elements[1].appendChild(div);
}, 3000);
